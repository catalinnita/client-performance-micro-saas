...
const mapping = {
    ...
    'Main Thread': {
        ...
        'Parse HTML': (data) => data["mainthread-work-breakdown"]["details"]["items"].filter(item => item.group === "parseHTML")[0]['duration'],   
        'Style Layout': (data) => data["mainthread-work-breakdown"]["details"]["items"].filter(item => item.group === "styleLayout")[0]['duration'],   
        'Rendering': (data) => data["mainthread-work-breakdown"]["details"]["items"].filter(item => item.group === "paintCompositeRender")[0]['duration'],   
    },
    'NextJs': {
    ...
    'Resources (transfer size - in bytes)': {
        'Document': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "document")[0]['transferSize'],
        'Script': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "script")[0]['transferSize'],
        'Stylesheet': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "stylesheet")[0]['transferSize'],
        'Image': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "image")[0]['transferSize'],
        'Font': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "font")[0]['transferSize'],
        'Media': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "media")[0]['transferSize'],
        'Other': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "other")[0]['transferSize'],
        'Total': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "total")[0]['transferSize'],
        'Third party': (data) => data["resource-summary"]["details"]["items"].filter(item => item.resourceType === "third-party")[0]['transferSize'],
    }
